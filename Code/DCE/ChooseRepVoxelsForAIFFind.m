...
MinVoxelsPerCluster=10;
if(max(Ns)<MinVoxelsPerCluster)
    MinVoxelsPerCluster=min(MinVoxelsPerCluster,min(Ns));
end
for i=1:max(Clust)
    CurI  = find(Clust==i);
    % If there are not enough voxels in this cluster (<100), continue
    if(size(CTC2D,1)>50 && numel(CurI)<MinVoxelsPerCluster)
        continue;
    end
    if(isempty(CurI))
        continue;
